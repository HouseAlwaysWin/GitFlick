...
- **The History branch filter is split into LOCAL and REMOTE sections, divided by a line.** Two checkboxes at the top show or hide each section, so a repo with a wall of remote-tracking branches can hide that half and work with just its local ones (or vice versa). Hiding a section keeps its ticks — they still filter.
