...
- **The History branch filter is split into LOCAL and REMOTE sections.** Each collapses on its own, so a repo with a wall of remote-tracking branches can fold that half away and work with just its local ones (or vice versa). Folding a section keeps its ticks — they still filter.
